...
pretrain_name = f"pretrain_ly6_ep50of50_{model_num}.pkl"
scales = (3, 9, 15)
max_len = 512
# number of encoder layers in use
encoder_nlayer = 6
# number of encoder layers frozen
freeze_nlayer = 5

lr, wd = 1e-5, 1e-3
batch_sz, train_ep = 16, 50
decay_it, lr_decay = 1e3, 0.95

layer = len(mainclasses.split('.')) if mainclasses else 0
...
class_head = f"sub{mainclasses}" if mainclasses else "main"
...
if mainclasses:
    ...
    classbranch = [label2ix[tuple(ecs[:i + 1])] for i in range(layer)]
